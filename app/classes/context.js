...
class Context {
    ...
    check_for_duty_drawback() {
        ...
        if ((this.scheme_code != null) && (this.scheme_code != null)) {
            const fs = require('fs');
            var filename = path.join(process.cwd(), "app", "data", "roo", "uk", "articles", this.scheme_code, "duty_drawback.md");
            // const path = './file.txt';
            var a = 1;
            try {
    ...
    get_article(document_type) {
        ...
        const fs = require('fs');
        var path = process.cwd() + '/app/views/roo_new/components/master_reference.html';
        ...
        try {
            ...
            link_to_original_content = link_to_original_content.replace(/{{ ARTICLE }}/g, match_inner);
            link_to_original_content = link_to_original_content.replace(/{{ ORD_TITLE }}/g, this.ord["ord_title"]);
            link_to_original_content = link_to_original_content.replace(/{{ ORD_VERSION }}/g, this.ord["ord_version"]);
            link_to_original_content = link_to_original_content.replace(/{{ ORD_DATE }}/g, this.ord["ord_date"]);
            link_to_original_content = link_to_original_content.replace(/{{ URL }}/g, "/public/downloads/roo_reference/" + this.ord["ord_original"]);
    ...
    get_cumulation_methods() {
        ...
        if (this.cumulation_method_count > 0) {
            for (const [key, value] of Object.entries(this.cumulation_methods)) {
                ...
                var eu_index = this.cumulation_methods[key].country_names.indexOf("0the European Union Member States");
                if (this.cumulation_methods[key].country_names.includes("0the European Union Member States")) {
                    this.cumulation_methods[key].country_names[eu_index] = "the European Union Member States";
                    var a = 1;
                }

                this.cumulation_methods[key].country_name_list = this.items_to_list(this.cumulation_methods[key].country_names);
                this.cumulation_methods[key].description_text = this.cumulation_texts[key];
                this.cumulation_methods[key].example_text = this.cumulation_examples[key];
                var a = 1;
            }
        }
        var a = 1;
    ...
    get_cumulation_texts() {
        ...
        this.cumulation_texts["regional"] = fs.readFileSync(path + "regional.md", 'utf8');
        this.cumulation_texts["full"] = fs.readFileSync(path + "full.md", 'utf8');

        this.cumulation_examples["bilateral"] = fs.readFileSync(path + "bilateral_example.md", 'utf8');
        this.cumulation_examples["diagonal"] = fs.readFileSync(path + "diagonal_example.md", 'utf8');
        this.cumulation_examples["extended"] = fs.readFileSync(path + "extended_example.md", 'utf8');
        this.cumulation_examples["regional"] = fs.readFileSync(path + "regional_example.md", 'utf8');
    ...
    get_all_psrs_for_scheme(req) {
        ...
        this.psrs["rule_sets"].forEach(psr => {
            ...
            // psr.subdivision = psr.subdivision.replace("<p><p>", "<p>")
            psr["rules"].forEach(rule => {
                a = 1;
                rule.rule = md.render(rule.rule);
                rule.rule = rule.rule.replace("<ul>", "<ul class='govuk-list govuk-list--bullet govuk-list--bullet-s'>")
            });
        });
        var a = 1;
    }
