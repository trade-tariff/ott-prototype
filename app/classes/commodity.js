...
class Commodity {
  ...
  get_data (json, country = null, date = null) {
    ...
    this.goods_nomenclature_item_id =
      this.data['attributes']['goods_nomenclature_item_id']
  ...
  anglicise_percentages () {
    this.description = this.description.replace(/\s%/gi, '%')
  ...
  anglicise_americanisations () {
    this.description = this.description.replace(/ized/gi, 'ised')
    this.description = this.description.replace(/izing/gi, 'ising')
    this.description = this.description.replace(/ization/gi, 'isation')
  }
  ...
  insert_description_hyperlinks () {
    ...
    this.description = this.description.replace(
      /(chapter)\s([0-9])$/gi,
      '[$1 $2](https://www.trade-tariff.service.gov.uk/chapters/0$2)'
    )
    this.description = this.description.replace(
      /(chapter)\s([0-9])([\s,;\)])/gi,
      '[$1 $2](https://www.trade-tariff.service.gov.uk/chapters/0$2)$3'
    )

    // 2 digit chapter string
    this.description = this.description.replace(
      /(chapter)\s([0-9]{2})$/gi,
      '[$1 $2](https://www.trade-tariff.service.gov.uk/chapters/$2)'
    )
    this.description = this.description.replace(
      /(chapter)\s([0-9]{2})([\s,;\)])/gi,
      '[$1 $2](https://www.trade-tariff.service.gov.uk/chapters/$2)$3'
    )

    // 4 digit headings at the end of the string
    this.description = this.description.replace(
      /\s([0-9]{4})$/g,
      ' [$1](https://www.trade-tariff.service.gov.uk/search?q=$1)'
    )

    // 4 digit headings followed by a comma or other punctuation
    this.description = this.description.replace(
      /\s([0-9]{4})([,;\)])/gi,
      ' [$1](https://www.trade-tariff.service.gov.uk/search?q=$1)$2'
    )

    // 4 digit headings followed by a space, but not numbers (which might be a sub heading)
    this.description = this.description.replace(
      /\s([0-9]{4})([\s])([^0-9])/gi,
      ' [$1](https://www.trade-tariff.service.gov.uk/search?q=$1$2)$2$3'
    )

    // Sub heading
    this.description = this.description.replace(
      /\s([0-9]{4})\s([0-9]{2})\s([0-9]{2})$/gi,
      ' [$1 $2 $3](https://www.trade-tariff.service.gov.uk/search?q=$1$2$3)'
    )
    this.description = this.description.replace(
      /\s([0-9]{4})\s([0-9]{2})\s([0-9]{2})([\s,;\)])/gi,
      ' [$1 $2 $3](https://www.trade-tariff.service.gov.uk/search?q=$1$2$3)$4'
    )
    this.description = this.description.replace(
      /\s([0-9]{4})\s([0-9]{2})$/gi,
      ' [$1 $2](https://www.trade-tariff.service.gov.uk/search?q=$1$2)'
    )
    this.description = this.description.replace(
      /\s([0-9]{4})\s([0-9]{2})([\s,;\)])/gi,
      ' [$1 $2](https://www.trade-tariff.service.gov.uk/search?q=$1$2)$3'
    )
  }
  ...
  check_for_references () {
    ...
    // Check for headings
    const regex_heading = /\sheading\s[0-9]{4}/gi
    ...
    const regex_subheading = /\ssubheading\s[0-9]{6,8}/gi
  ...
  get_measure_data (
    ...
    this.included.forEach(item => {
      if (item['type'] == 'section') {
        ...
        var description = item['attributes']['formatted_description']
        this.ancestry.push(description)
        this.ancestry_ids.push(
          item['attributes']['goods_nomenclature_item_id'] + '_80'
        )
        ...
        this.ancestry_ids.push(
          item['attributes']['goods_nomenclature_item_id'] +
            '_' +
            item['attributes']['producline_suffix']
        )
        ...
        m.measure_type = new MeasureType(
          item['relationships']['measure_type']['data']
        )
        m.geographical_area_id =
          item['relationships']['geographical_area']['data']['id']
        ...
        var excluded_countries =
          item['relationships']['excluded_countries']['data']
        ...
        var measure_conditions =
          item['relationships']['measure_conditions']['data']
  ...
  sort_ancestry_ids () {
    ...
    this.gnidpls =
      this.goods_nomenclature_item_id + '_' + this.productline_suffix
  ...
  assign_additional_codes () {
      ...
      this.additional_codes.forEach(ac => {
        if (m.additional_code_id == ac.id) {
          ...
          if (control_series.includes(m.measure_type_series_id)) {
            if (
              this.measure_types_geographical_areas_and_acs[unique_identity] ===
              undefined
            ) {
              this.measure_types_geographical_areas_and_acs[unique_identity] = [
                ac.code.substr(2, 2)
    ...
    this.measures.forEach(m => {
      ...
      m.additional_code_message =
        control_text + ' applies to goods covered under '
      var unique_identity = m.measure_type_id + '_' + m.geographical_area_id
      if (
        this.measure_types_geographical_areas_and_acs[unique_identity] !==
        undefined
        ...
        if (obj.includes('49') || obj.includes('98') || obj.includes('99')) {
          ...
          if (others.includes(s)) {
            m.additional_code_message =
              control_text + ' does not apply to goods covered under '
          }
        }
      }
    })
      ...
      this.excises.forEach(excise => {
        ...
        m.measure_components.forEach(component => {
          if (component.measurement_unit_code != null) {
            if (
              !this.excise_supp_unit_requirement.includes(
                component.measurement_unit_code
              )
            ) {
              this.excise_supp_unit_requirement.push(
                component.measurement_unit_code
              )
            }
            // break
